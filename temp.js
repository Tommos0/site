...
var loadScript = function(url) { return new Promise(function(resolve, reject) { var script = document.createElement('script'); script.addEventListener('load', resolve); script.addEventListener('error', reject); script.src = url; document.head.appendChild(script); }); };
...
loadCSS('reset.css');
loadCSS('main.css?13');
getFile('program.js').then(function(script) {
    document.body.innerHTML = '';
    jsBlock = createCodeElm(script);
    document.body.appendChild(jsBlock);

    j1 = Object.create(jsmachine);
    j1.lines = script.split("\n");
    j1.display = function (line) { return new Promise(function(resolve) { highlight(document.querySelector('pre'),j1.currentLine.toString()); resolve(); })};
    j1.processLine(80);
